import { faArrowRight, faClockRotateLeft, faHourglassHalf, faRetweet } from '@fortawesome/free-solid-svg-icons';
...
const DeploymentStateCard = ({ record, title, icon, color }) => {
	...
	const dateValue = differenceInMinutes(parseJSON(record.achievement_horizon_end_date), new Date()) /
		differenceInMinutes(parseJSON(record.achievement_horizon_end_date), parseJSON(record.achievement_horizon_start_date))
		...
		<div className="card surface-50 shadow-1 p-3 border-1 border-50 border-round">
			...
			<div className="mr-5 align-items-center mt-3 ">
				...
				<div className="flex justify-content-between flex-wrap card-container purple-container">
					<div className="flex align-items-center justify-content-center text-blue">{record.achievement_horizon_start_date}</div>
					<div className="flex align-items-center justify-content-center text-blue">{record.achievement_horizon_end_date}</div>
				</div>
			</div>
		</div>
